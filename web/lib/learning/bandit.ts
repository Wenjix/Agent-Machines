 ...
 * per-class posterior is used only once its cell clears a sample threshold τ,
...
/** Sample threshold τ: a per-task_class cell is trusted only at/above this. */
...
/** Pick the aggregate for an arm: per-class if it clears τ, else global. */
...
/** Beta(α,β) for an arm's success, seeded by the substrate benchmark prior. */
